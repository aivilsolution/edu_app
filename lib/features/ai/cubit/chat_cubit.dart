...
class ChatCubit
  ...
  bool _isTemporaryChat =
      false;
  static const _maxTitleLength =
      40;
  ...
  initialize() async {
    try {
      ...
      final temporaryChat =
          _repository.createTemporaryChat();
      _isTemporaryChat =
          true;
      await _loadChatWithProvider(
        temporaryChat,
        [],
      );
    } catch (
  ...
  Future<
    void
  >
  _initializeRepository() async {
    _repository =
        await ChatRepository.forCurrentUser;
  }
  ...
  >
  _cleanup() async {
    _currentProvider?.removeListener(
      _onProviderHistoryChanged,
    );
  }
  ...
  deleteChat(
    ...
    try {
      ...
      final currentState =
          state
              as ChatLoadedState;
      if (currentState.currentChat.id ==
          chat.id) {
  ...
  void _onProviderHistoryChanged() {
    ...
    if (_isTemporaryChat &&
        _currentProvider!.history.isNotEmpty) {
      _persistTemporaryChat(
        currentState.currentChat,
      );
  ...
  _updateChatHistory(
    ...
    try {
      final history =
          _currentProvider!.history.toList();
      if (!_isTemporaryChat) {
  ...
  _persistTemporaryChat(
    ...
    try {
      final persistedChat = await _repository.addChat(
        temporaryChat:
            chat,
      );
      _isTemporaryChat =
      ...
      emit(
        ChatErrorState(
          'Failed to persist temporary chat',
          e,
          stackTrace,
        ),
      );
