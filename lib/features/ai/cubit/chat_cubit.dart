...
class ChatCubit extends Cubit<ChatState> {
  ...
  Future<void> updateChatTitle(Chat chat, String newTitle) async {
    if (state is! ChatLoadedState || newTitle.trim().isEmpty) return;
  ...
  void _onProviderHistoryChanged() {
    if (state is! ChatLoadedState || _currentProvider == null) return;
  ...
  bool _shouldGenerateTitle(Chat chat, List<ChatMessage> history) {
    ...
    return userMessages.isNotEmpty && history.length <= 5;
  }
  ...
  Future<void> _generateChatTitle(
    ...
    try {
      debugPrint('Generating title for chat ${chat.id}');
      ...
      if (userMessages.isEmpty) {
        debugPrint('No user messages to generate title from');
        return;
      }
